<?php
require_once __DIR__ . '/inc/single-page-navwalker/single-page-navwalker.php';
include_once __DIR__ . "/inc/globals.php";

/**
 * Single Page Override
 * This overrides Page Template inclusions
 *
 */
add_filter('template_include', 'the_single_page_template_override');

function the_single_page_template_override($template)
{
    global $post;
...
function get_the_loop($menuname = 'main-menu')
{
    $args = array(
        'sort_order' => 'DESC',
    ...
    if ($the_query->have_posts()) :
        while ( $the_query->have_posts() ) : $the_query->the_post();
            ...
            if (!$template || $template == 'default') $template = 'page-templates/template-default.php';
...
add_filter('admin_init', 'the_general_settings_logo_field');
function the_general_settings_logo_field() {
    register_setting('general', 'logo', 'esc_attr');
    add_settings_field('logo', '<label for="logo">'.__('Logo' , THE_TEXT_DOMAIN ).'</label>' , 'the_general_logo_field', 'general');
}

function the_general_logo_field() {
    $logo = get_option('logo');
    ob_start();
    ?>
    <button type="button" class="button button-default button-media" data-title="Select or upload Website Logo">Add Logo</button>
    <img src="<?php echo $logo ?>" class="logo-preview">
...
add_action('admin_head',            'the_enqueue_admin_scripts');
...
function the_enqueue_styles()
{
    ...
    wp_register_style('fullpage', get_template_directory_uri() . '/vendor/fullPage.js/dist/jquery.fullpage.min.css', array(), '2.1.1', 'all');
...
function the_enqueue_admin_scripts()
{
    wp_enqueue_script('media-upload');
    wp_enqueue_media();
    wp_register_script('admin', get_template_directory_uri() . '/js/admin.js', array('jquery'), '1.3.3', true);
    wp_enqueue_script('admin');
}
...
{
    wp_register_script('jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js', array('jquery'), '1.3.3', true);
    ...
    wp_register_script('scrollOverflow', get_template_directory_uri() . '/vendor/fullPage.js/vendors/scrolloverflow.min.js', array('jquery'), '3.5.0', true);
    wp_enqueue_script('scrollOverflow');

    wp_register_script('fullpage', get_template_directory_uri() . '/vendor/fullPage.js/dist/jquery.fullpage.min.js', array('jquery'), '3.5.0', true);
    ...
    wp_register_script('script', get_template_directory_uri() . '/js/script.js', array('jquery'), THE_VERSION, true);
...
function the_main_nav($id = "", $class = "", $isSinglePage = true)
{
    wp_nav_menu(array(
        ...
        'walker'          => $isSinglePage ? new SinglePageNavwalker() : '',
    ));
}
...
function the_linking_nav($class = "")
{
    wp_nav_menu(array(
        'theme_location'  => 'main-menu',
        'menu'            => '',
        ...
        'depth'           => 2,
        'fallback_cb'     => 'bs4navwalker::fallback',
        'walker'          => new SinglePageNavwalker()
    ));
}
...
// Contact Metaboxes
function singlepage_add_meta_boxes_page() {
    global $post;
    if ('page-templates/template-contact.php' == get_post_meta($post->ID, '_wp_page_template', true)) {
        add_meta_box(
            'singlecontactinfometabox', // $id
            'Contact Info', // $title
            function () {
                global $post;
                require("views/partials/nonce.php");

                $old = get_post_meta($post->ID, 'singlecontactinfometabox', true);

                require("views/metabox/contactinfo.php");
            }, // $callback
            'page', // $page
            'normal', // $context
...
add_action( 'add_meta_boxes_page', 'singlepage_add_meta_boxes_page' );